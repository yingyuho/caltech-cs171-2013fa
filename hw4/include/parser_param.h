...
public:
    ...
    SeparatorParam() : cPtr(0), nPtr(0), iPtr(0), mPtr(0) {}
    ...
    virtual Separator* make_object() const {
        if (!incomplete_param())
            return new Separator(transforms, mPtr, cPtr, nPtr, iPtr);
        else
            return 0;
    }
};
...
struct TransformParam : public ParserParam<TRSTransform> {
public:
    Translation *t;
    Rotation *r;
    ScaleFactor *s;
    ...
    virtual TRSTransform* make_object() const {
        TRSTransform *tr = new TRSTransform();
        if (t) tr->t = *t;
        if (r) tr->r = *r;
        if (s) tr->s = *s;
        return tr;
...
public:
    ...
    MaterialParam() : defShininess(0.2), aColor(0), dColor(0), sColor(0), shininess(0) {
