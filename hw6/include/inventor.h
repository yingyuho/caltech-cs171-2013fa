...
typedef MeshBuilder<Vec2> MBTCoord;
...
class Separator : \
public MBCoord, public MBNorml, public MBTCoord, \
public CoordTransformer, public NormalTransformer 
{
public:
    Material * mPtr;
    std::vector<Vec3> * coord3Ptr;
    std::vector<Vec3> * normalPtr;
    std::vector<Vec2> * tcoord2Ptr;
    ...
    virtual void process_mesh(Mesh<Vec2>& mesh, const std::vector<Vec2>& pMap) const;

    const std::vector<Vec4> get_obj_space_coord() const throw ( NullReferenceException );
    ...
    const std::vector<NVec3> get_obj_space_normal() const throw ( NullReferenceException );
    const std::vector<NVec3> get_world_space_normal() const;

    const std::vector<Vec2> get_texture_coord() const throw ( NullReferenceException );
    ...
    const Material& get_material() const throw ( NullReferenceException );
...
class IndexedFaceSet : public MBCoord, public MBNorml, public MBTCoord {
public:
    std::vector<int> * coordIndex;
    std::vector<int> * normalIndex;
    std::vector<int> * tCoordIndex;
public:
    ...
    const std::vector<int>& get_coord_index() const throw ( NullReferenceException );
    const std::vector<int>& get_normal_index() const throw ( NullReferenceException );
    const std::vector<int>& get_text_coord_index() const throw ( NullReferenceException );
};
