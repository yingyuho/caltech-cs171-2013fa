...
void Inventor::process_mesh(Mesh<Vec4>& mesh) const {
    ...
    // for each separator
    for ( auto it = sepList.cbegin(); it != sepList.cend(); it++ ) {
...
const PtrVector<PointLight>& Inventor::get_lights() const { return plList; }
const PtrVector<Separator>& Inventor::get_separators() const { return sepList; }
...
bool Inventor::validate_index() const {
    // for each separator
    for ( auto it = sepList.begin(); it != sepList.end(); it++ )
    { 
        if ( ! (*it)->validate_index() ) return false;
    }
    return true;
}
...
// Separator::Separator(const PtrVector<TRSTransform>& transforms, Material * mPtr\
// , Coordinate3* cPtr, Normal3* nPtr, IndexedFaceSet* iPtr) \
// : mPtr(mPtr), coord3Ptr(cPtr), normalPtr(nPtr), ifsPtr(iPtr), transforms(transforms) {}
...
Mat44 Separator::to_left_matrix() const {
    Mat44 result(1.);
    for ( auto &tr : transforms )
        result = result * tr->to_left_matrix();
    return result;
}

Mat33 Separator::to_right_matrix() const {
    Mat33 result(1.);
    for ( auto &tr : transforms )
        result = tr->to_right_matrix() * result;
    return result;
}
...
MAKE_METHOD_SEP_BUILD_POLYGON_LIST(Vec2);

const std::vector<Vec4> Separator::get_obj_space_coord() const throw ( NullReferenceException ) {
    if ( coord3Ptr == 0 ) throw NullReferenceException();

    const int L = coord3Ptr->size();
    std::vector<Vec4> result(L);

    for (int i = 0; i < L; i ++)
    {
        result[i][0] = coord3Ptr->at(i)[0];
        result[i][1] = coord3Ptr->at(i)[1];
        result[i][2] = coord3Ptr->at(i)[2];
        result[i][3] = 1.;
    }
...
const std::vector<Vec4> Separator::get_norm_device_coord(const PerspectiveCamera& pc) const {
    ...
    // P * C^(-1) = pcMatrix
    // O = object transforms = tPtr->to_left_matrix()
    Mat44 mat = pc.to_left_matrix() * to_left_matrix();
...
const std::vector<IntVec2> Separator::get_pixel_coord(const Mat44& pcMatrix\
    ...
    // P * C^(-1) = pcMatrix
    // O = object transforms = tPtr->to_left_matrix()
    Mat44 mat = pcMatrix * to_left_matrix();
...
const std::vector<NVec3> Separator::get_obj_space_normal() const throw ( NullReferenceException ) {
    if ( normalPtr == 0 ) throw NullReferenceException();

    const int L = normalPtr->size();
    std::vector<NVec3> result(L);

    for (int i = 0; i < L; i ++)
    {
        result[i] = normalPtr->at(i).transpose();
    }
...
const std::vector<Vec2> Separator::get_texture_coord() const throw ( NullReferenceException )
{
    if ( tcoord2Ptr == 0 ) throw NullReferenceException();

    const int L = tcoord2Ptr->size();
    std::vector<Vec2> result(L);

    for (int i = 0; i < L; i ++)
    {
        result[i][0] = tcoord2Ptr->at(i)[0];
        result[i][1] = tcoord2Ptr->at(i)[1];
    }
...
/*const TRSTransform& Separator::get_transform() const {
    return *tPtr;
}*/

const Material& Separator::get_material() const throw ( NullReferenceException ) {
    if (mPtr)
        return *mPtr;
    else
        throw NullReferenceException("Separator::get_material()");
}
...
IndexedFaceSet::~IndexedFaceSet() {
    ...
    if (tCoordIndex) delete tCoordIndex;
}
...
MAKE_METHOD_IFS_BUILD_POLYGON_LIST(Vec2, tCoordIndex);

const std::vector<int>& IndexedFaceSet::get_coord_index() const throw ( NullReferenceException )
{ 
    if (coordIndex)
        return *coordIndex;
    else
        throw NullReferenceException("IndexedFaceSet::get_coord_index()");
}

const std::vector<int>& IndexedFaceSet::get_normal_index() const throw ( NullReferenceException )
{
    if (normalIndex)
        return *normalIndex;
    else
        throw NullReferenceException("IndexedFaceSet::get_normal_index()");
}

const std::vector<int>& IndexedFaceSet::get_text_coord_index() const throw ( NullReferenceException )
{
    if (tCoordIndex)
        return *tCoordIndex;
    else
        throw NullReferenceException("IndexedFaceSet::get_text_coord_index()");
}
